...
// const data = [...Array(72).fill()].map((_, i) => ({ id: i }));
...
const CustomUsePagination = ({ count }) => {
  const data = [...Array(count).fill()].map((_, i) => ({ id: i }));
    ...
    >
      <ButtonBase {...items.shift()} className={classes.boxArrow} >
        <Icon style={{ display: "flex", justifyContent: "center" }}>
          <img src={LeftTriangleIcon} style={{ width: "16PX" }} alt="caret left icon" />
        </Icon>
      </ButtonBase>
      <Box className={classes.box}>{page}</Box>
      <ButtonBase {...items.pop()} className={classes.boxArrow}>
        <Icon style={{ display: "flex", justifyContent: "center" }}>
          <img src={RightTriangleIcon} style={{ width: "16PX" }} alt="caret left icon" />
        </Icon>
      </ButtonBase>
      <Box>
...
export default CustomUsePagination;